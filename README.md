...
Accept the self-signed certificate warning in your browser (the cert simulates AWS ACM in production, see the migration plan for what changes).
...
## Security model: firewall rules table

Each service can only talk to what it needs:

| Source           | Destination       | Port  | Reason for allowing connection                |
|------------------|-------------------|-------|-----------------------------------------------|
| Host             | web01             | 80    | Redirects to HTTPS                            |
| web01            | app01             | 8080  | Sits in front  and forward requests to Tomcat |
| app01            | db01              | 3306  | MariaDB (database)                            |
| app01            | mc01              | 11211 | Memcached (data in memory)                    |
| app01            | rmq01             | 5672  | RabbitMQ (queue background jobs)              |
| everyone else    | anything backend  | -     | denied by firewalld                           |

## AWS migration

This is a mini version of production AWS. When you move to the cloud, you're replacing each local component with an AWS service that does the same thing:
...
The full migration plan, with cost estimate and Terraform pseudocode, is in [`docs/aws-migration-plan.md`]. Google Cloud Platform (GCP) service mapping is in [`docs/gcp-equivalence.md`].
